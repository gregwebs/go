...
func (n *ErrorHandlerExpr) Format(s fmt.State, verb rune) { fmtNode(n, s, verb) }
func (n *ErrorHandlerExpr) copy() Node {
	c := *n
	c.init = copyNodes(c.init)
	return &c
}
func (n *ErrorHandlerExpr) doChildren(do func(Node) bool) bool {
	if doNodes(n.init, do) {
		return true
	}
	if n.X != nil && do(n.X) {
...
func (n *ErrorHandlerExpr) editChildren(edit func(Node) Node) {
	editNodes(n.init, edit)
	if n.X != nil {
		n.X = edit(n.X).(Node)
	}
